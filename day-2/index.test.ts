...
describe("Day 2", () => {
    ...
    describe("isSafeReport()", () => {
        ...
        describe("with dampening", () => {
            it("returns `true` for the report [7, 6, 4, 2, 1]", () => {
                const result = isSafeReportWithDampening([7, 6, 4, 2, 1]);

                expect(result).toBe(true);
            });

            it("returns `false` for the report [1, 2, 7, 8, 9]", () => {
                const result = isSafeReportWithDampening([1, 2, 7, 8, 9]);

                expect(result).toBe(false);
            });

            it("returns `false` for the report [9, 7, 6, 2, 1]", () => {
                const result = isSafeReportWithDampening([9, 7, 6, 2, 1]);

                expect(result).toBe(false);
            });

            it("returns `true` for the report [1, 3, 2, 4, 5]", () => {
                const result = isSafeReportWithDampening([1, 3, 2, 4, 5]);

                expect(result).toBe(true);
            });

            it("returns `true` for the report [8, 6, 4, 4, 1]", () => {
                const result = isSafeReportWithDampening([8, 6, 4, 4, 1]);

                expect(result).toBe(true);
            });

            it("returns `true` for the report [1, 3, 6, 7, 9]", () => {
                const result = isSafeReportWithDampening([1, 3, 6, 7, 9]);
